...
export interface CreateResumeRes {
...
function createBaseCreateResumeRes(): CreateResumeRes {
  return { resume: undefined };
}

export const CreateResumeRes: MessageFns<CreateResumeRes> = {
  encode(message: CreateResumeRes, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): CreateResumeRes {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseCreateResumeRes();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): CreateResumeRes {
    return { resume: isSet(object.resume) ? Resume.fromJSON(object.resume) : undefined };
  },

  toJSON(message: CreateResumeRes): unknown {
    const obj: any = {};
    if (message.resume !== undefined) {
  ...
  create<I extends Exact<DeepPartial<CreateResumeRes>, I>>(base?: I): CreateResumeRes {
    return CreateResumeRes.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<CreateResumeRes>, I>>(object: I): CreateResumeRes {
    const message = createBaseCreateResumeRes();
