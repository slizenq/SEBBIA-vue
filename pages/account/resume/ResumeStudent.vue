<template>
	<div class="container">
		...
		<div>
			...
			Добавить фото
			<el-popover placement="top-start" title="Добавляй свое фото" :width="300" trigger="hover" content="Это гарантирует лучшее запоминание твоего резюме среди других претендентов">
...
const selectedEducation = computed(() => {
  return (
    educationOptions.find((option) => option.value === education.value) || null
  );
});
...
const sendForm = async function () {
	...
	try {
		...
		try {
			const getResumeResponse = await axios.post(`${IP}/createStudent`);
			console.log("get запрос успешен:", getResumeResponse);
		} catch (error) { console.error( "Ошибка при выполнении get запроса:", error.response ? error.response.data : error.message)}
		var postData = {
			first_name: "Виктор",
			...
			photo: "",
			directions: ["string"],
			location: selectedCity.value?.label || resumeData?.location || "",
			portfolio: "http://example.com",
		};
		try {
			const postResponse = await axios.post(`${IP}/createStudent`, postData);
			console.log("PUT запрос успешен:", postResponse);
		} catch (error) { console.error("Ошибка при выполнении PUT запроса:", error.response ? error.response.data : error.message)}
		} else {
		const response = await axios.post(`${IP}/createStudent`, studentEntity);
		let data = { resume_id: response.data?.resume_id, progress: 0 };
